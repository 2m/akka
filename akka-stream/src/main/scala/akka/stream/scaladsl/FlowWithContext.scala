...
final class FlowWithContext[-In, -CtxIn, +Out, +CtxOut, +Mat](delegate: Flow[(In, CtxIn), (Out, CtxOut), Mat])
  ...
  def asJava[JIn <: In, JCtxIn <: CtxIn, JOut >: Out, JCtxOut >: CtxOut, JMat >: Mat]
    ...
    new javadsl.FlowWithContext(javadsl
      .Flow.create[Pair[In, CtxIn]]()
      .map(_.toScala)
      .viaMat(delegate.map { case (first, second) => Pair(first, second) }.asJava, javadsl.Keep.right[NotUsed, JMat]))
}
